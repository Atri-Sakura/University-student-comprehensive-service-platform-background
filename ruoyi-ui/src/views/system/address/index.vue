<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="商家ID" prop="merchantBaseId">
        <el-input
    ...
    <el-table v-loading="loading" :data="addressList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="地址ID" align="center" prop="merchantAddressId" />
      <el-table-column label="商家ID" align="center" prop="merchantBaseId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="商家ID" prop="merchantBaseId">
          <el-input v-model="form.merchantBaseId" placeholder="请输入商家ID" />
        </el-form-item>
        <el-form-item label="省份" prop="province">
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.merchantAddressId)
    ...
    handleUpdate(row) {
      this.reset()
      const merchantAddressId = row.merchantAddressId || this.ids
      getAddress(merchantAddressId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.merchantAddressId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const merchantAddressIds = row.merchantAddressId || this.ids
      this.$modal.confirm('是否确认删除商家地址编号为"' + merchantAddressIds + '"的数据项？').then(function() {
        return delAddress(merchantAddressIds)
      }).then(() => {
        this.getList()
