  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="冻结金额" prop="freezeAmount">
        <el-input
          v-model="queryParams.freezeAmount"
          placeholder="请输入冻结金额"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="walletList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="钱包唯一ID" align="center" prop="merchantWalletId" />
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.merchantWalletId)
    ...
    handleUpdate(row) {
      this.reset()
      const merchantWalletId = row.merchantWalletId || this.ids
      getWallet(merchantWalletId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.merchantWalletId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const merchantWalletIds = row.merchantWalletId || this.ids
      this.$modal.confirm('是否确认删除商家钱包编号为"' + merchantWalletIds + '"的数据项？').then(function() {
        return delWallet(merchantWalletIds)
      }).then(() => {
        this.getList()
