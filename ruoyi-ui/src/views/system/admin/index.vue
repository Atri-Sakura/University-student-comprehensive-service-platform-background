<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="关联角色ID" prop="platformRoleId">
        <el-input
          v-model="queryParams.platformRoleId"
          placeholder="请输入关联角色ID"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="adminList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="管理员唯一ID" align="center" prop="platformAdminId" />
      <el-table-column label="登录账号" align="center" prop="username" />
      <el-table-column label="密码" align="center" prop="password" />
      <el-table-column label="真实姓名" align="center" prop="realName" />
      <el-table-column label="联系电话" align="center" prop="phone" />
      <el-table-column label="关联角色ID" align="center" prop="platformRoleId" />
      <el-table-column label="账号状态：0-禁用 1-正常" align="center" prop="accountStatus" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="关联角色ID" prop="platformRoleId">
          <el-input v-model="form.platformRoleId" placeholder="请输入关联角色ID" />
        </el-form-item>
        <el-form-item label="最后登录时间" prop="lastLoginTime">
          <el-date-picker clearable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        platformRoleId: [
          { required: true, message: "关联角色ID不能为空", trigger: "blur" }
        ],
        accountStatus: [
          { required: true, message: "账号状态：0-禁用 1-正常不能为空", trigger: "change" }
        ],
        createTime: [
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.platformAdminId)
    ...
    handleUpdate(row) {
      this.reset()
      const platformAdminId = row.platformAdminId || this.ids
      getAdmin(platformAdminId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.platformAdminId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const platformAdminIds = row.platformAdminId || this.ids
      this.$modal.confirm('是否确认删除平台管理员编号为"' + platformAdminIds + '"的数据项？').then(function() {
        return delAdmin(platformAdminIds)
      }).then(() => {
        this.getList()
