<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="announcementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="公告唯一ID" align="center" prop="platformAnnouncementId" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        status: [
          { required: true, message: "状态：0-草稿 1-已发布 2-已下架不能为空", trigger: "change" }
        ],
        readCount: [
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.platformAnnouncementId)
    ...
    handleUpdate(row) {
      this.reset()
      const platformAnnouncementId = row.platformAnnouncementId || this.ids
      getAnnouncement(platformAnnouncementId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.platformAnnouncementId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const platformAnnouncementIds = row.platformAnnouncementId || this.ids
      this.$modal.confirm('是否确认删除系统公告编号为"' + platformAnnouncementIds + '"的数据项？').then(function() {
        return delAnnouncement(platformAnnouncementIds)
      }).then(() => {
        this.getList()
