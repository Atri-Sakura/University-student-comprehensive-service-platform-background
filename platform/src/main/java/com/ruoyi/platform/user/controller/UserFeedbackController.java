...
public class UserFeedbackController extends BaseController
{
    ...
    public TableDataInfo list(UserFeedback userFeedback)
    {
        ...
        // 强制设置为当前用户，防止越权
        userFeedback.setUserType(1); // 用户类型
        userFeedback.setUserId(userBaseId);
        startPage();
    ...
    public AjaxResult getInfo(@PathVariable("feedbackId") Long feedbackId)
    {
        ...
        // 验证是否是自己的反馈
        if (!feedback.getUserId().equals(userBaseId) || feedback.getUserType() != 1) {
    ...
    public AjaxResult edit(@RequestBody UserFeedback userFeedback)
    {
        ...
        // 验证是否是自己的反馈
        if (!existingFeedback.getUserId().equals(userBaseId) || existingFeedback.getUserType() != 1) {
